...
// SIFT es 50 veces más lenta pero obtiene 7 veces más puntos igualados
// FAST detecta más puntos que SURF
// STAR / MSER generan muy pocos puntos clave

#define DETECTOR_TYPE	"FAST" // FAST, SIFT, SURF, STAR, MSER, GFTT, HARRIS ... ver la función crear
...
void GetPair(Mat &imgL, Mat &imgR, vector<Point2f> &ptsL, vector<Point2f> &ptsR) 
{
	...
	for(size_t i1 = 0; i1 < ptsLtemp.size(); i1++)
	{
		...
		if(abs(ptsRtemp[i1].x - prjPtR.x) < HOMO_FILTER_TH &&
			abs(ptsRtemp[i1].y - prjPtR.y) < 2) // La restricción es más estricta
		{
